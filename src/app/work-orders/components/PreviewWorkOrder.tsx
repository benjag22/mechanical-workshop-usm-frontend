...
export default function PreviewWorkOrder({ ...props }: TrimmedWorkOrder) {
  ...
  const handleAuthorize = async (rut: string): Promise<boolean> => {
    try {
      const response = await api.isAuthorized({
        path: { id: props.id, rut: rut },
      });
      ...
      if (response.data === true) {
        setTimeout(() => {
          router.push(`/work-orders/detail/${props.id}`);
        }, 1500);
        return true;
      }
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <h3 className="text-white font-semibold text-lg">Orden #{props.id}</h3>
            <span
              className={cn(
          ...
          <span className="text-slate-400 text-sm">{formatDate(props.createdAt)}</span>
        </div>
        ...
        <div className="mb-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-slate-200 font-medium">
              {props.mechanicLeaderFullName ?? "Sin asignar"}
            </span>
          </div>
        </div>
